...
import { WalletsAction, WalletState } from "../reducers/wallets";
    ...
    (action: AnyAction) => {
      if (supportsCalculator(action)) {
        ...
        if (recalculate) {
          ...
          try {
            const earnings = calculateEarningsAndInfo(
              config,
  ...
  const depositsOutOfPocket = Object.entries(walletEarningsMap).reduce(
    (accumDepositsOutOfPocket, [walletId, walletEarnings]) => {
      ...
      const totalDeposits = Object.values(wallet?.monthInputs ?? {}).reduce(
        (accumTotalDeposits, { deposits }) => {
          return (
            accumTotalDeposits +
            deposits.reduce((accumMonthDeposits, deposit) => {
...
function calculateMonthEarnings(
  ...
  return (accum: Record<number, MonthEarningsAndInfo>, date: Date) => {
    ...
    const earningDaysInMonth = totalDaysInMonth - date.getDate();
...
function getLastCustomDripValue(
  ...
  return monthDates.length > 0
    ? wallet.monthInputs[monthDates.length - 1].dripValue
    : fallbackDripValue;
}
